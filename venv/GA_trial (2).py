...
init_population = ["00000010000", "01010100101",
                   "10101011010", "00111100000",
                   "01000010101", "10010101010",
                   "00101010000", "01111100101",
                   "10000011010", "00010100000"]
...
crossover_points = [3, 5, 7, 8, 10]


def crossover_updated(population_old, fitness_population_old, selected_population):
    offspring = [""] * 2
    selected_pt = random.choice(crossover_points) - 1  # since points are from 1 to 11 and array index start with 0
...
def split_str(str, j):
    ...
    min_list[1] = bp_value(str1[2] + str1[3] + "", j)
    min_list[2] = chol_value(str1[4] + str1[5] + "", j)
    min_list[3] = singlebit_value(str1[6] + "", j, 'blood_sugar')
    min_list[4] = heartrate_value(str1[7] + str1[8] + "", j)
    min_list[5] = st_value(str1[9] + str1[10] + "", j)
    return min_list
...
def print_all_pop(final_pop, support_val):
    for i in range(0, len(final_pop)):
        ...
        print('Population = ', pop, ' Support = ', support_val[i], ' Support % = ',
              (support_val[i] * 100) / float(df.shape[0]))
...
def print_pop(population):
    ...
    blood = double_bit_conv(population[2] + population[3] + "")
    chol = double_bit_conv(population[4] + population[5] + "")
    sugar = population[6] + ""
    heart = double_bit_conv(population[7] + population[8] + "")
    st = st_bit_conv(population[9] + population[10] + "")
    pop = "Age = ", age, " Blood Pressure = ", blood, " Cholesterol = ", chol, " Blood Sugar = ", sugar, " Heart Rate = ", heart, " ST Depression = ", st, ""
    return pop
